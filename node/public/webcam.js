...
function procesarCamara() {
	...
	for (var i = 0; i < pixeles.length; i += 4) {
		...
		if (distancia < distanciaAceptableColor) {//pintamos pixeles
			pixeles[i] = 0;//r
			pixeles[i + 1] = 255;	//g
			pixeles[i + 2] = 0; //b
			...
			//Agrupacion
			if (elementos.length == 0) {// mi primer objeto
	...
	for (var k = 0; k < elementos.length; k++) {	//si el area de un objeto es muy pequeño no lo consideramos

		var width = elementos[k].xMaxima - elementos[k].xMinima;
		var height = elementos[k].yMaxima - elementos[k].yMinima;
		var area = width * height;
		if (area > 800) {
			if (objMasGrande == null || area > masGrande) {//buscamos el objeto de mayor tamaño
	...
	if (objMasGrande != null) {
		...
		enviarGrados(objMasGrande.grados);
	}
...
function unirRectIntesect(elementos) {
	...
	for (var i = 0; i < elementos.length; i++) {
		for (var j = 0; j < elementos.length; j++) {
			...
			if (intersectan) {//pasar por los pixeles del j al i
...
/*La función envía una solicitud a una dirección URL específica utilizando jQuery.
-El parámetro "grados" se utiliza para determinar el valor de la variable "movimiento".
-Si "grados" es mayor o igual a 20, "movimiento" se establece en "-1".
-Si "grados" es menor o igual a -20, "movimiento" se establece en "1".
-Se construye una URL utilizando el valor de "movimiento".
-Se comprueba si la URL es diferente a la URL previamente revisada.
-Si la URL es diferente, se realiza una solicitud HTTP GET a la URL utilizando la biblioteca jQuery.
-La respuesta de la solicitud se registra en la consola del navegador.
-La variable "revisaUrl" se actualiza para guardar la URL revisada más reciente*/ 
var revisaUrl = null;

function enviarGrados(grados) {
	...
	var url = "http://localhost:8000?movimiento=" + movimiento;

	if (revisaUrl == null || url != revisaUrl) {
		revisaUrl = url;
